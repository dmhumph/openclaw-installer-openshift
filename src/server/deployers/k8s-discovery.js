...
async function loadSavedNamespaces() {
    try {
...
function deriveInstanceStatus(replicas, readyReplicas, pods) {
    if (replicas === 0) {
...
export async function discoverK8sInstances(options = {}) {
    const results = [];
    try {
        const core = coreApi();
        const apps = appsApi();
